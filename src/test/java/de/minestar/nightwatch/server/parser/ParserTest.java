...
public class ParserTest {
	...
	public void version17WithoutErrors() {
		...
		ServerLogEntry entry = parser.parse(today,
				"[12:42:35] [Server thread/INFO]: Generating keypair");
		...
		entry = parser.parse(today,
				"[11:13:46] [Netty IO #2/INFO]: Client protocol version 1");
		...
		entry = parser
				.parse(today,
						"[11:15:07] [Server thread/INFO]: Meldanor[/192.168.1.33:62038] logged in with entity id 2666 at ([world] -88.5, 67.0, 174.5)");
		logTime = LocalTime.of(11, 15, 07);
		assertEquals(logTime.atDate(today), entry.getTime());
		assertEquals("Server thread", entry.getSource());
		assertEquals(LogLevel.INFO, entry.getLogLevel());
		assertEquals(
				"Meldanor[/192.168.1.33:62038] logged in with entity id 2666 at ([world] -88.5, 67.0, 174.5)",
				entry.getText());
	...
	public void version17InvalidFormat() {
		assertFalse(new Version1710Parser()
				.accepts("2014-07-21 20:43:48 [Information] [ForgeModLoader] Forge Mod Loader version 6.4.49.965 for Minecraft 1.6.4 loading"));
	}
	...
	public void testForge16Parser() {
		...
		ServerLogEntry entry = parser
				.parse(LocalDate.now(),
						"2014-08-26 21:02:42 [INFO] Starting minecraft server version 1.6.4");
		...
		entry = parser
				.parse(LocalDate.now(),
						"2014-08-26 21:03:19 [INFO] [Universal Electricity] Injected universalelectricity.core.asm.template.item.TemplateTEItem API into: resonantinduction/electrical/battery/ItemBlockBattery");
		timeStamp = LocalDateTime.of(2014, 8, 26, 21, 03, 19);
		assertEquals(timeStamp, entry.getTime());
		assertEquals(
				"Injected universalelectricity.core.asm.template.item.TemplateTEItem API into: resonantinduction/electrical/battery/ItemBlockBattery",
				entry.getText());
		assertEquals(LogLevel.INFO, entry.getLogLevel());
	...
	public void testCauldronConsoleParser() {
		...
		ServerLogEntry entry = parser.parse(LocalDate.now(),
				"[23:48:23 INFO]: Generating keypair");
		...
		entry = parser
				.parse(LocalDate.now(),
						"[23:48:23 WARN]: Offendor: org/spigotmc/RestartCommand.restart()V");
		time = LocalTime.of(23, 48, 23);
		assertEquals(time.atDate(LocalDate.now()), entry.getTime());
		assertEquals("Offendor: org/spigotmc/RestartCommand.restart()V",
				entry.getText());
