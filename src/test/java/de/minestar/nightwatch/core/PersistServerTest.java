...
public class PersistServerTest {
	...
	public void saveAndLoadSerers() throws IOException {
		...
		manager.registeredServers().put(
				"java8server",
				new ObservedServer("Java8Server", new File("path/to/server"),
						"1024MB", "2G", true, false));
		manager.registeredServers().put(
				"java7server",
				new ObservedJava7Server("Java7Server", new File(
						"path/to/other/server/"), "2G", "4G", false, true,
						"256MB"));

		manager = new ServerManager(tmpFile);
		ObservableMap<String, ObservedServer> registeredServers = manager
				.registeredServers();

		ObservedServer observedServerOne = registeredServers.get("java8server");
		assertTrue("not an instance of ObseredServer",
				observedServerOne instanceof ObservedServer);
		assertEquals("Java8Server", observedServerOne.getName());
		assertEquals(new File("path/to/server").getAbsolutePath(),
				observedServerOne.getServerFile().getAbsolutePath());
		...
		assertTrue("not an instance of ObseredJava7Server",
				observedServerTwo instanceof ObservedJava7Server);
		assertEquals("Java7Server", observedServerTwo.getName());
		assertEquals(new File("path/to/other/server").getAbsolutePath(),
				observedServerTwo.getServerFile().getAbsolutePath());
		assertEquals("2G", observedServerTwo.getMinMemory());
		assertEquals("4G", observedServerTwo.getMaxMemory());
		assertEquals("256MB",
				((ObservedJava7Server) observedServerTwo).getPermGenSize());
