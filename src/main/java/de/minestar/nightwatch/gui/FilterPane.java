...
public class FilterPane extends FlowPane {
	...
	private void createGUI() {
		this.getStylesheets().add(
				getClass().getResource("/styles/datetextfield.css")
						.toExternalForm());
		...
		fromDateTextField.dateTimeProperty().addListener(
				(observ, oldValue, newValue) -> {
					logFilter
							.setToDateFilter(p -> p.getTime().isEqual(newValue)
									|| p.getTime().isAfter(newValue));
				});

		this.toDateTextField = new DateTimeTextField(MainGUI.GERMAN_FORMAT);
		toDateTextField.setPrefWidth(120);

		toDateTextField.dateTimeProperty().addListener(
				(observ, oldValue, newValue) -> {
					logFilter.setToDateFilter(p -> p.getTime()
							.isEqual(newValue)
							|| p.getTime().isBefore(newValue));
				});

		Button resetStartDateButton = new Button(null, new ImageView(new Image(
				getClass().getResourceAsStream("/icons/revert16.png"))));
		resetStartDateButton.setOnAction(e -> {
			fromDateTextField.setText(minDate.format(MainGUI.GERMAN_FORMAT));
		});

		Button resetEndDateButton = new Button(null, new ImageView(new Image(
				getClass().getResourceAsStream("/icons/revert16.png"))));
		resetEndDateButton.setOnAction(e -> {
			toDateTextField.setText(maxDate.format(MainGUI.GERMAN_FORMAT));
		});

		ChoiceBox<LogLevel> minimumlogLevel = new ChoiceBox<LogLevel>(
		...
		minimumlogLevel.valueProperty().addListener(
				(observ, oldValue, newValue) -> {
					logFilter.setMinimumLevelFilter(p -> newValue.compareTo(p
							.getLogLevel()) <= 0);
				});
		...
		filterText.textProperty().addListener(
				(value, oldValue, newValue) -> {
					logFilter.setContainsTextFilter(p -> p.getText().contains(
							newValue));
				});

		this.getChildren().addAll(new Label("From"), fromDateTextField,
				resetStartDateButton, new Label("To"), toDateTextField,
				resetEndDateButton, new Separator(Orientation.VERTICAL),
				new Label("Level >= "), minimumlogLevel,
				new Label("Search Text"), filterText);

	}

	public void bindDateInterval(
			ReadOnlyObjectProperty<LocalDateTime> minBinding,
			ReadOnlyObjectProperty<LocalDateTime> maxBinding) {
		...
		minBinding
				.addListener((observ, oldVal, newVal) -> {
					this.minDate = newVal;
					this.fromDateTextField.setText(newVal
							.format(MainGUI.GERMAN_FORMAT));
				});
	...
	public class LogFilter implements Predicate<ServerLogEntry>,
			ObservableValue<LogFilter> {
		...
		public LogFilter() {
			fromDateFilter = toDateFilter = containsTextFilter = minimumLevelFilter = (
					p) -> true;
		...
		public void setContainsTextFilter(
				Predicate<ServerLogEntry> containsTextFilter) {
			this.containsTextFilter = containsTextFilter;
			informObservers();
		}

		public void setMinimumLevelFilter(
				Predicate<ServerLogEntry> minimumLevelFilter) {
		...
		public boolean test(ServerLogEntry t) {
			return fromDateFilter.and(toDateFilter).and(containsTextFilter)
					.and(minimumLevelFilter).test(t);
		}
