...
public class BackupTask extends Task<Void> {
	...
	public static final DateTimeFormatter BACKUP_TIME_FORMAT = DateTimeFormatter
	...
	protected Void call() throws Exception {
		...
		procededFiles.addListener((observ, oldVal, newVal) -> {
			...
			updateMessage("Backup file " + procededFiles.get() + " of "
					+ filesCount);
		});

		String timestampString = LocalDateTime.now().format(BACKUP_TIME_FORMAT);
		File targetFile = new File(backupDirectory, server.getName() + "_"
				+ timestampString + ".zip");
