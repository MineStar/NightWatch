...
public class ServerOverwatchThread extends Task<Void> {
	...
	public ServerOverwatchThread(ObservedServer server,
			List<ServerLogEntry> logList,
	...
	private void start() throws Exception {
		...
		this.logTask = new ServerLoggingTask(serverProcess.getInputStream(),
				logList);
		Thread loggingTaskThread = new Thread(logTask, this.server.getName()
				+ "_LoggingTask");

		this.commandTask = new ServerCommandTask(
				serverProcess.getOutputStream(), commandQueue);
		Thread commandTaskThread = new Thread(commandTask,
				this.server.getName() + "_CommandTask");
