...
public class ServerManager {
	...
	public ServerManager(File serversFile) {
		...
		registeredServers
				.addListener((MapChangeListener<String, ObservedServer>) c -> {
					saveServers(serversFile, registeredServers);
				});
	}

	private ObservableMap<String, ObservedServer> loadServers(File file) {
		ObservableMap<String, ObservedServer> result = FXCollections
				.observableHashMap();
		...
		try {
			Map<String, ObservedServer> list = Core.JSON_MAPPER.readValue(file,
					SERVER_TYPE);
	...
	private void saveServers(File file, Map<String, ObservedServer> servers) {
		try {
			Core.JSON_MAPPER.writerWithType(SERVER_TYPE).writeValue(file,
					servers);
		} catch (Exception e) {
			e.printStackTrace();
		}
